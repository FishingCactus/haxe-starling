package com.greensock;
   ...
   class FlexBlitMask extends UIComponent
   {
      ...
      public function new(target:DisplayObject = null, x:Float = 0, y:Float = 0, width:Float = 100, height:Float = 100, smoothing:Bool = false, autoUpdate:Bool = true, fillColor:UInt = 0, wrap:Bool = false)
      {
         super();
      ...
      override public  function set_rotation(value)
      {
         if(value != 0)
         {
            throw new openfl.errors.Error("Cannot set the rotation of a FlexBlitMask to a non-zero number. FlexBlitMasks should remain unrotated.");
         }
      }
         ...
         if(_bitmapMode != value)
         {
            ...
            if(_target != null)
            {
               ...
               if(_bitmapMode)
               {
                  ...
                  {
                     this.blendMode = cast(_target.alpha == 0, Bool) || cast(_target.alpha == 1, Bool)?BlendMode.NORMAL:BlendMode.LAYER;
                  }
                  else
                  {
